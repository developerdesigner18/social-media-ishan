...
export default function UserStory({selectedUser,selectedUsername,selectedUserImage}) {
  ...
  useEffect(()=>{
    axios.post('http://localhost:5000/story/getstory',{id:selectedUser},{
      headers:{
              ...
              return(
                <Carousel.Item key={index}>
                  ...
                  <div style={{display:'flex',margin:'10px 0px 0px 10px'}} >
                    <Avatar src={`http://localhost:5000/static/${selectedUserImage}`} sx={{height:'40px',width:'40px',border:'2px solid #2E7D32'}} />
